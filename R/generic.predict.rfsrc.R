           ...
           ...)
{
  ...
  if (!restore.mode) {
    ...
    if (!setequal(xfactor$factor, newdata.xfactor$factor)) {
      ...
      stop("z-variable factors from test data do not match original training data")
    }
    if (!setequal(xfactor$order, newdata.xfactor$order)) {
      ...
      stop("(ordered) z-variable factors from test data do not match original training data")
    }
    ...
    if (length(xvar.names) != sum(is.element(xvar.names, names(newdata)))) {
      ...
      stop("z-variables in test data do not match original training data")
    }
