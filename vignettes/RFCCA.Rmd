...
# Introduction
This R package implements the Random Forest with Canonical Correlation Analysis (RFCCA) method proposed by REFERENCE_TO_OUR_PAPER (link to the paper?). The theoretical details are given in the next section. The last section presents a data analysis using this method. 
...
For further data analysis, one can use SHAP values (Lundberg et al., 2017) to gain additional insights. SHAP values show how much each variable contributes, either positively or negatively, to the individual predictions. For an example of application to the problem in question, see REFERENCE_TO_OUR_PAPER.
