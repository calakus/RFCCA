...
# Introduction
This R package implements the Random Forest with Canonical Correlation Analysis (RFCCA) method proposed by .... (link to the paper?). The theoretical details are given in the next section. The last section presents a data analysis using this method. 
...
# Data analysis
We will show how to use RFCCA package on a generated data set. The data set has three sets of variables: $X \in R^{nx2}$, $Y \in R^{nx2}$ and $Z \in R^{nx7}$. The sample size ($n$) is 300. The correlation between $X$ and $Y$ depends only on $Z_1,...,Z_5$ (*i.e.* $Z_6 and Z_7$ are noise variables). Firstly, we load the data and split the data into train and test sets:
...
Then, we apply the global significance test to evaluate the global effect of $Z$. 
```{r}
...
Next, we train the random forest with `rfcca()`. We can get the out-of-bag (OOB) predictions from the grow object.
```{r}
...
```{r, fig.show='hold', fig.width=6, fig.height=4, fig.align='center'}
plot.vimp(vimp.obj)
```
...
Besides the canonical correlation estimations, we can get the estimated canonical coefficients for the $X$ and $Y$, $a$ and $b$.
```{r}
